...
-- def i1 (x : PowerBasis 𝒪[K] 𝒪[L]) (a : L ≃ₐ[M] L) (ha : a ∈ Finset.univ) : { y // y ∈ (Polynomial.map (algebraMap ↥𝒪[M] ↥𝒪[L]) (minpoly (↥𝒪[M]) x.gen)).roots } := ⟨⟨a x.gen, aux_12 x a⟩, aux_13 x a⟩
...
-- -- #check Multiset.card_eq_card_of_rel
-- -- #check roots_mem_integralClosure
-- -- def i3 (x : PowerBasis 𝒪[K] 𝒪[L]) : { x_1 // x_1 ∈ (Polynomial.map (algebraMap ↥𝒪[M] ↥𝒪[L]) (minpoly (↥𝒪[M]) x.gen)).roots.toFinset } ≃ { x_1 // x_1 ∈ (Polynomial.map (algebraMap (↥𝒪[M]) L) (minpoly (↥𝒪[M]) x.gen)).roots.toFinset } := {
-- --       toFun := by
-- --         intro ⟨t, ht⟩
-- --         refine ⟨t, ?_⟩
-- --         simp only [Multiset.mem_toFinset, ← Polynomial.aroots_def] at ht ⊢
-- --         rw [mem_aroots] at ht
-- --         rw [mem_aroots']
-- --         constructor
-- --         · apply (Polynomial.map_ne_zero_iff _).2 ht.1
-- --           have : algebraMap 𝒪[M] L = (algebraMap M L) ∘ (algebraMap 𝒪[M] M) := rfl
-- --           rw [this]
...
--   #check IntermediateField.fixingSubgroupEquiv
...
--       sorry
--       -- this theorem exist in new version
--       --simp [fixingSubgroup_bot]
--     rw [this]
...
--   have hprod : ∏ x_1 : L ≃ₐ[M] L, (X - C (⟨x_1 ↑x.gen, aux_12 x x_1⟩ : 𝒪[L])) = ∏ t : { y // y ∈ (Polynomial.map (algebraMap 𝒪[M] 𝒪[L]) (minpoly 𝒪[M] x.gen)).roots}, (X - C t.1) := by
...
--           have hlead : f.leadingCoeff = 1 := by sorry
--             -- simp only [f]
...
--           let s := i '' .univ
--           have hroots : ∏ t ∈ (⊤ : Set (L ≃ₐ[M] L)).toFinset, (X - C (t x.gen)) = ∏ t ∈ s.toFinset, (X - C t) := by sorry
--             -- simp only [s, i]
...
-- --       exact ha
-- --       exact restrictScalarsHom_injective ↥𝒪[K]
--           simp only [hroots, Polynomial.roots_prod_X_sub_C, s, i, Set.image_univ, Set.toFinset_range, Finset.image_val]
--           simp only [Multiset.esymm, map_multiset_sum, Multiset.map_map, Function.comp_apply, map_multiset_prod]
--           congr 1
--           symm
...
--                 use σ.symm * l
--                 constructor
--                 · exact Finset.mem_univ_val (σ.symm * l)
...
--         exact ⟨m, hm⟩
--       have heq : ∃ f' : M[X], Polynomial.map (algebraMap M L) f' = f := by
...
--             -- intro t ht
--             -- simp only [s, Multiset.mem_map, Finset.mem_val, Finset.mem_univ, true_and] at ht
--             -- obtain ⟨a, ha⟩ := ht
--             -- rw [← ha]
--             -- have : (algebraMap (↥𝒪[M]) L) = (algebraMap 𝒪[L] L).comp (algebraMap (↥𝒪[M]) 𝒪[L]) := rfl
--             -- simp only [this, ← Polynomial.map_map]
--             -- apply Multiset.mem_of_le
--             -- apply Polynomial.map_roots_le
--             -- simp only [Polynomial.map_map]
--             -- apply (Polynomial.map_ne_zero_iff _).2
--             -- apply minpoly.ne_zero
--             -- exact IsIntegral.isIntegral x.gen
...
--             -- refine Multiset.mem_map.mpr ?_
--             -- use (⟨a x.gen, aux_12 x a⟩ : 𝒪[L])
--             -- refine ⟨h, rfl⟩
--             repeat sorry
--             --apply Multiset.mem_of_le Polynomial.map_roots_le
--             -- simp only [mem_roots', ne_eq, IsRoot.def, eval_map_algebraMap]
--             -- constructor
--             -- · by_contra hc
--             --   have h1 : minpoly 𝒪[M] x.gen = 0 := by
--             --     apply_fun Polynomial.map (algebraMap ↥𝒪[M] L)
--             --     simp only [Polynomial.map_zero]
--             --     exact hc
--             --     apply Polynomial.map_injective
--             --     have : (algebraMap 𝒪[M] L) = (algebraMap 𝒪[L] L) ∘ (algebraMap 𝒪[M] 𝒪[L]) := rfl
--             --     rw [this]
--             --     apply Function.Injective.comp
--             --     exact IsFractionRing.injective (↥𝒪[L]) L
--             --     exact IsValExtension.integerAlgebra_injective M L
...
--               -- rcases b with ⟨b, hb1⟩
--               -- simp only [mem_roots', ne_eq, IsRoot.def, eval_map_algebraMap] at hb1
--               -- simp only [aeval_def, this, eval₂_def] at hb1 ⊢

--               -- rcases b with ⟨b, hb1⟩
--               -- simp only [aeval_def, this, eval₂_def] at hb1 ⊢
--               -- have hmem : a ∈ decompositionGroup M L := mem_decompositionGroup a
--               -- conv =>
--               --   enter [1, 1, 1]
--               --   rw [← DecompositionGroup.restrictValuationSubring_apply' hmem]
...
--             symm
--             by_contra hc
--             have h2 : (Polynomial.map (algebraMap (↥𝒪[M]) L) (minpoly (↥𝒪[M]) x.gen)).roots ⊆ s := subset_of_subset_of_eq (fun ⦃a⦄ a ↦ a) hc
--             exact h1 h2
...
--       absurd hlt
--       push_neg
--       have hmp : (minpoly M (algebraMap 𝒪[L] L x.gen)) = Polynomial.map (algebraMap 𝒪[M] M) (minpoly 𝒪[M] x.gen) := by
--         apply minpoly.isIntegrallyClosed_eq_field_fractions
--         exact IsIntegral.isIntegral x.gen
...
--       intro i hi
--       exact monic_X_sub_C (i ↑x.gen)
--       simp only [aeval_def, eval₂_eq_eval_map, hf', f, eval_prod, eval_sub, eval_C, eval_X]
--       simp only [Set.top_eq_univ, Set.toFinset_univ]
--       apply Finset.prod_eq_zero_iff.2
--       use .refl
--       constructor
--       · exact Finset.mem_univ AlgEquiv.refl
--       · exact sub_eq_zero_of_eq rfl
--     · intro a ha
--       simp only [i1]
--   repeat sorry
--   -- simp only [Set.top_eq_univ, Set.toFinset_univ, hprod, Finset.prod_eq_multiset_prod]
--   -- have hmap :  (Multiset.map (fun a ↦ X - C a) (Polynomial.map (algebraMap ↥𝒪[M] ↥𝒪[L]) (minpoly (↥𝒪[M]) x.gen)).roots) =  (Multiset.map (fun a ↦ X - C a.1) (@Finset.univ { y // y ∈ (Polynomial.map (algebraMap ↥𝒪[M] ↥𝒪[L]) (minpoly (↥𝒪[M]) x.gen)).roots }).val) := by
--   --   -- apply Multiset.map_congr
--   --   -- (f := (fun a ↦ X - C a)) (g := (fun t ↦ X - C t.1)) (s := (Polynomial.map (algebraMap ↥𝒪[M] ↥𝒪[L]) (minpoly (↥𝒪[M]) x.gen)).roots) (t := (@Finset.univ { y // y ∈ (Polynomial.map (algebraMap ↥𝒪[M] ↥𝒪[L]) (minpoly (↥𝒪[M]) x.gen)).roots }).val)
...
--   -- --rw [Polynomial.natDegree_map_eq_of_injective, Polynomial.natDegree_le_iff_degree_le]
--   -- have h1 : (minpoly M (algebraMap 𝒪[L] L x.gen)) = Polynomial.map (algebraMap 𝒪[M] M) (minpoly 𝒪[M] x.gen) := by
--   --   apply minpoly.isIntegrallyClosed_eq_field_fractions
--   --   exact IsIntegral.isIntegral x.gen
--   -- have h2 : (algebraMap M L).comp (algebraMap 𝒪[M] M) = algebraMap 𝒪[M] L := rfl
--   -- have h3 : Multiset.card (Polynomial.map (algebraMap ↥𝒪[M] ↥𝒪[L]) (minpoly (↥𝒪[M]) x.gen)).roots = Multiset.card (Polynomial.map (algebraMap 𝒪[M] L) (minpoly (↥𝒪[M]) x.gen)).roots := by
--   --   apply Multiset.card_eq_card_of_rel (r := fun a b => a.1 = b)
--   --   refine Multiset.rel_map_left.mp ?_
--   --   refine Multiset.rel_eq.mpr ?_
--   --   ext t
...
--   --       simp only [Multiset.mem_map, mem_roots', IsRoot.def, eval_map_algebraMap,Subtype.exists, exists_and_right, exists_and_left, exists_eq_right] at hc
--   --       simp only [mem_roots', IsRoot.def, eval_map_algebraMap]
...
--   --         exact IsValExtension.integerAlgebra_injective M L
--   --       · obtain ⟨hc2, hc2'⟩ := hc2
--   --         rw [← Subtype.val_inj, ZeroMemClass.coe_zero] at hc2'
--   --         rw [← hc2']
...
--   --       simp only [mem_roots', IsRoot.def, eval_map_algebraMap] at hcon
--   --       rcases hcon with ⟨hc1, hc2⟩
--   --       constructor
--   --       · rw [Polynomial.map_ne_zero_iff] at hc1 ⊢
--   --         exact hc1
--   --         exact IsValExtension.integerAlgebra_injective M L
...
--   --   apply Polynomial.Separable.map
--   --   apply Algebra.IsSeparable.isSeparable
--   --   -- rw [← Multiset.toFinset_card_eq_card_iff_nodup.2, ← Multiset.toFinset_card_eq_card_iff_nodup.2]
--   --   -- apply Finset.card_eq_of_equiv (i3 x)
--   --   -- apply nodup_roots
--   --   -- apply Polynomial.Separable.map
--   --   -- apply Algebra.IsSeparable.isSeparable
--   --   -- apply nodup_roots
--   --   -- apply Polynomial.Separable.map
--   --   -- apply Algebra.IsSeparable.isSeparable
--   -- simp only [h3, ← h2, ← Polynomial.map_map, ← h1]
...
--   -- #check exists_prod_multiset_X_sub_C_mul
--   -- #check minpoly.isIntegrallyClosed_eq_field_fractions
--   -- have : (minpoly M (algebraMap 𝒪[L] L x.gen)) = Polynomial.map (algebraMap 𝒪[M] M) (minpoly 𝒪[M] x.gen) := by
--   --   apply minpoly.isIntegrallyClosed_eq_field_fractions
--   --   exact IsIntegral.isIntegral x.gen
--   -- obtain ⟨q, he, hd, hr⟩ := exists_prod_multiset_X_sub_C_mul (Polynomial.map (algebraMap ↥𝒪[M] ↥𝒪[L]) (minpoly (↥𝒪[M]) x.gen))
--   -- rw [← hd]
--   -- symm
...
--   -- have hcoeff : (((Polynomial.map (algebraMap M L) (minpoly M x.gen.1))).coeffs : Set L) ⊆ 𝒪[L] := by sorry
--   -- have heq : (Polynomial.map (algebraMap 𝒪[M] 𝒪[L]) (minpoly 𝒪[M] x.gen)) = (Polynomial.toSubring (Polynomial.map (algebraMap M L) (minpoly M x.gen.1)) 𝒪[L] hcoeff) := by
--   --   sorry
--   -- rw [heq]
--   -- simp only [natDegree_toSubring]
--   -- apply Polynomial.splits_iff_card_roots.1
--   -- #check Polynomial.natDegree_eq_card_roots'
--   -- have hcard : (Polynomial.map (algebraMap 𝒪[M] L) (minpoly 𝒪[M] x.gen)).natDegree = Multiset.card (Polynomial.map (algebraMap 𝒪[M] L) (minpoly 𝒪[M] x.gen)).roots := by
--   --   apply Polynomial.natDegree_eq_card_roots'
--   --   #check Normal.splits
--   --   sorry
--   -- #check Polynomial.natDegree_map_eq_of_injective
--   -- #check Polynomial.splits_iff_card_roots
--   -- rw [Polynomial.natDegree_map_eq_of_injective (IsValExtension.integerAlgebra_injective M L), ← Polynomial.natDegree_map_eq_of_injective (f := algebraMap 𝒪[M] L), hcard]
--   -- apply le_antisymm
--   -- ·
--   --   sorry
--   -- · sorry
--   -- apply Multiset.card_eq_card_of_rel (r := fun a b => a.1 = b)
--   -- refine Multiset.rel_map_left.mp ?hroots.a
--   -- refine Multiset.rel_eq.mpr ?hroots.a.a
--   -- ext x
--   -- #check Polynomial.count_roots
...
-- -- #check Polynomial.eq_prod_roots_of_monic_of_splits_id
-- -- #check PowerBasis.liftEquiv'
-- -- variable (x : PowerBasis 𝒪[K] 𝒪[L])
-- -- #check PowerBasis.liftEquiv' x
-- -- theorem aux_19 (x : PowerBasis 𝒪[K] 𝒪[L]) : Polynomial.map (algebraMap 𝒪[M] 𝒪[L]) (minpoly 𝒪[M] x.gen) = ∏ t ∈ (⊤ : Set (L ≃ₐ[M] L)).toFinset, (X - C (⟨t x.gen, aux_12 x t⟩ : 𝒪[L])) := by
...
-- --     · intro b hb
-- --       simp only [i1]
-- --       let x' : PowerBasis 𝒪[M] 𝒪[L] := {
-- --         gen := x.gen
-- --         dim := sorry
-- --         basis := sorry
-- --         basis_eq_pow := sorry
-- --       }
-- --       let a':= x'.liftEquiv'.symm b

-- --       -- have hmem : b.1 ∈ (minpoly 𝒪[K] x.gen).aroots 𝒪[L] := by sorry
-- --       -- let b' := (⟨b.1, hmem⟩ :  { y // y ∈ (minpoly (𝒪[K]) x.gen).aroots ↥𝒪[L]})
-- --       -- let a':= x.liftEquiv'.symm b'
-- --       -- let a : L ≃ₐ[M] L := {
-- --       --   toFun := by sorry
-- --       --   invFun := sorry
...
-- --       simp only [mem_roots', ne_eq, IsRoot.def, eval_map_algebraMap] at hb
-- --       rcases hb with ⟨hb1, hb2⟩
-- --       simp only [aeval_def] at hb2
...
-- --     exact IsIntegral.isIntegral x.gen
-- --   · apply le_antisymm
-- --     · exact card_roots' (minpoly (↥𝒪[M]) x.gen)
-- --     · -- · #check Polynomial.natDegree_eq_card_roots'
-- --     --   #check Polynomial.natDegree_map_eq_of_injective
-- --     --   #check Polynomial.ofSubring
-- --     --   let i : 𝒪[M] →+* M := {
-- --     --     toFun := fun x => (x : M)
-- --     --     map_one' := rfl
-- --     --     map_mul' := fun x y => rfl
...
-- --     --     map_add' := fun x y => rfl
-- --     --   }
-- --     --   have : Splits i (minpoly (𝒪[M]) x.gen) := by
-- --     --     simp only [i]
